<div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>{{ $latestSubscription?->currency?->currency_symbol }}{{ number_format($latestSubscription?->currentInvoice?->total ?? 0, 2) }}</div>
    ...
    <form wire:submit="updatePackageSubmit" class="space-y-4">
        ...
        @if($packageType === App\Enums\PackageType::TRIAL || $billingCycle === 'trial')
            <div>
                <x-label for="trial_expire_on" value="{{ __('modules.package.trialExpireOn') }}" />
                <x-datepicker type="date" id="trial_expire_on" class="w-full mt-1" wire:model.blur="trialExpireOn" autocomplete="off" placeholder="{{ __('modules.package.selectDate') }}" />
        ...
        @else
            <div>
                ...
                <div class="relative mt-1 rounded-md">
                    <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                        <span class="text-gray-500 dark:text-gray-400">{{ $selectedPackage?->currency?->currency_symbol }}</span>
                    </div>
                    <x-input id="amount" type="number" step="0.01" min="0" class="w-full pl-8" wire:model.blur="amount" />
                </div>
            ...
            <div>
                ...
                <x-input type="date" id="pay_date" class="w-full mt-1" wire:model.blur="payDate" autocomplete="off" placeholder="{{ __('modules.package.selectDate') }}" />
            ...
            <div>
                ...
                <x-input type="date" id="next_pay_date" class="w-full mt-1" wire:model.blur="nextPayDate" autocomplete="off" placeholder="{{ __('modules.package.selectDate') }}" />
                <x-input-error for="nextPayDate" class="mt-2" />
            </div>

            <div>
                <x-label for="licence_expires_on" value="{{ __('modules.package.licenceExpiresOn') }}" />
                <x-input type="date" id="licence_expires_on" class="w-full mt-1" wire:model.blur="licenceExpireOn" autocomplete="off" placeholder="{{ __('modules.package.selectDate') }}" />
